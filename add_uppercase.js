...
const Classes = [
    ...
    { match: /tracking-tight\s*text-xl(?! )/g, replace: 'tracking-tight  text-xl' },
    { match: /font-bold([^"]*text-text-primary[^"]*mb-2(?! ))/g, replace: 'font-bold$1  tracking-wider' }, // headers
    { match: /font-bold([^"]*text-text-primary[^"]*mb-4(?! ))/g, replace: 'font-bold$1  tracking-wider' }, // headers
]
...
files.forEach(file => {
    ...
    for (const { match, replace } of Classes) {
