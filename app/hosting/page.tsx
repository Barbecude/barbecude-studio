...
export default function HostingPage() {
  return (
    <div className="pb-16">
      ...
      <div className="bg-amber-500/10 border-b border-amber-500/30 text-amber-500 py-3.5 px-4 text-center text-xs tracking-wider font-bold tracking-wider flex items-center justify-center gap-2">
      ...
      <div className="relative bg-black/60 border-b border-stone-gray/30 py-24 px-4 flex flex-col items-center justify-center text-center overflow-hidden filter brightness-50 opacity-40">
        ...
        <div className="z-10 relative select-none">
          ...
          <h1 className="font-semibold tracking-tight text-4xl md:text-5xl lg:text-6xl text-stone-500 mb-6 tracking-tighter font-bold">
            Hosting Berkinerja <span className="text-stone-500">Tinggi</span>
          </h1>
          <p className="text-sm tracking-widest md:text-lg text-stone-600 max-w-2xl mx-auto leading-relaxed">
            Luncurkan server Minecraft instan kamu dengan garansi uptime 99.9%, penyimpanan cepat SSD NVMe, dan standar perlindungan DDoS.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 mt-16 select-none opacity-25 filter brightness-50 pointer-events-none">
        ...
        <div className="flex flex-col md:flex-row gap-6 items-end justify-center mb-16">
          {plans.map((plan) => (
            <div key={plan.name} className="relative flex-1 min-w-[220px] bg-bg-surface border border-stone-gray p-6 flex flex-col rounded-none">
              {plan.popular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-stone-700 text-bg-primary font-bold text-xs tracking-wider px-3 py-1 tracking-wider whitespace-nowrap border-2 border-bg-primary rounded-none">
                  Paling Populer
                </div>
              )}
              <h3 className={`font-semibold tracking-tight text-xl mb-4 ${plan.color}`}>{plan.name}</h3>

              <div className="mb-6">
                <div className="text-3xl text-text-primary font-bold">
                  <span className="text-lg">Rp</span>{(plan.price / 1000).toString()}rb<span className="text-sm tracking-widest text-text-secondary">/bln</span>
                </div>
              </div>

              <ul className="space-y-3 mb-8 flex-1 text-sm tracking-widest text-text-secondary border-t border-stone-gray pt-4">
              ...
              <span className="minecraft-btn-gold w-full text-center py-2.5 font-bold text-xs tracking-wider tracking-wider bg-bg-panel border-stone-gray text-stone-500 cursor-not-allowed">
                TIDAK TERSEDIA
              </span>
            </div>
          ))}
        ...
        <div className="minecraft-panel p-8 text-center max-w-4xl mx-auto">
          <h2 className="text-xl text-text-primary mb-6">Bandingkan Semua Fitur Lengkap</h2>
