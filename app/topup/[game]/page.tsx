...
export default function GameTopupDetail({ params }: { params: Promise<{ game: string }> }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
       <Link href="/topup" className="text-text-secondary hover:text-text-primary text-sm tracking-widest font-bold flex items-center gap-2 mb-6">
          ← Back to Games
       </Link>
          ...
          <div className="w-full md:w-1/3 flex flex-col gap-6">
             <div className="minecraft-panel p-6 flex items-center gap-4 border-l-4 border-l-purple-500">
                ...
                <div>
                   ...
                   <p className="text-xs tracking-wider text-text-secondary tracking-widest mt-1">Moonton</p>
                </div>
             </div>
             ...
             <div className="minecraft-panel p-6">
                ...
                <div className="flex gap-4">
                  <div className="flex-1">
                     <label className="text-xs tracking-wider font-bold text-text-secondary block mb-1">User ID</label>
                  ...
                  <div className="w-1/3">
                     <label className="text-xs tracking-wider font-bold text-text-secondary block mb-1">Zone ID</label>
                ...
                <button className="w-full mt-4 bg-stone-700 hover:bg-stone-600 text-text-primary font-bold py-2 border-2 border-stone-500 rounded-sm text-sm tracking-widest">
          ...
          <div className="w-full md:w-2/3 flex flex-col gap-6">
             <div className="minecraft-panel p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {items.map(item => (
                    ...
                    >
                      {item.promo && (
                         <div className="absolute -top-2 -right-2 bg-accent-gold text-bg-primary text-[10px] font-bold px-1.5 py-0.5 shadow-md z-10">
                           HOT
                         </div>
                      )}
                      <Gem className="w-8 h-8 mb-2 text-purple-300" />
                      <div className="font-bold text-sm tracking-widest mb-1 leading-tight">{item.name}</div>
                      <div className="text-text-primary text-xs tracking-wider">Rp {(item.price).toLocaleString('id-ID')}</div>
                    </button>
                  ))}
                </div>
             </div>
             ...
             <div className="minecraft-panel p-6 bg-purple-900/20 border-purple-500/50">
                <div className="flex items-center gap-4">
                   <div className="flex-1">
                      <div className="text-xs tracking-wider text-text-secondary font-bold mb-1">Promo Code (Optional)</div>
